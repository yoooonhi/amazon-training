#!/usr/bin/env node
/**
 * 迁移脚本：把「真实」运营实战经验（ops-experience）的 markdown 上传到
 * Supabase Storage 的 course-content bucket。
 *
 * 用法：
 *   1. 确保已建好 course-content bucket + RLS（见 supabase/migrations/002）
 *   2. 设置环境变量 SUPABASE_SERVICE_ROLE_KEY
 *      ⚠️ service_role key 拥有完全权限，切勿提交到 git！脚本读完后不会持久化。
 *   3. SUPABASE_SERVICE_ROLE_KEY=xxx node scripts/migrate-ops-experience.mjs
 *
 * 与 migrate-playbooks.mjs 同构，只改 LOCAL_DIR / STORAGE_PREFIX。
 * 可重复执行（upsert）。
 */
...
// 源目录：原始正文备份在 .archive/（content/ 下的会被改成壳）
const LOCAL_DIR = '.archive/playbooks-original/ops-experience'
// Storage 内的路径前缀（与壳的 storage_path 一一对应）
const STORAGE_PREFIX = 'playbooks/ops-experience'
...
async function main() {
  console.log(`迁移 ops-experience → Supabase Storage`)
